...
void Rotate(int p, int c)
{
    ...
    tree[x].pre = p;
    if(y)
    	if(x == tree[y].ch[0]) tree[y].ch[0] = p;
...
void Splay(int x)
{
    int top = 1;
    Stack[0] = x;
    for(int q = x; !IsRoot(q);)
    	Stack[top++] = (q = tree[q].pre);
    while(top)
    	PushDown(Stack[--top]);
    while(!IsRoot(x))
    {
...
{
    Splay(x);
    for(PushDown(x); tree[x].ch[0]; x = tree[x].ch[0])
    	PushDown(x);
    Splay(x);
    return x;
}
...
void Link(int x, int y)//link操作即为链接两个树, 那么要先进行expose操作, 把到路径上的边都变为实边, 这样才能进行把x调整到根部, 进而通过更改祖先来进行链接  
{
    ChangeRoot(x);
    Splay(x);
    tree[x].pre = y;
